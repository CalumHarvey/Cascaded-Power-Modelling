from ann import Fibre_ModelLoad
...
def trainModel(inputPowers, outputPowers, settings, perms, numTraining=300):
    ...
    fibre_output_df = pd.DataFrame(fibre_output)
    combinedInputs = pd.concat([fibre_output_df, settings[["0", "1", "2", "3"]]], axis=1)
    ...
    model.eval()
    edfa4_outputs = model(inputs_tensor)


    # FIBRE 4
    offChannelLoss, onChannelLoss, offChannelStd, onChannelStd = Fibre_ModelLoad.getLosses("fibre 4")
    fibre_output, _ = Fibre_ModelLoad.applyLosses(perms, offChannelLoss, onChannelLoss, offChannelStd, onChannelStd, edfa4_outputs.detach().cpu().numpy())
    ...
    EDFAID = "5"
    path = "models/ann/"
    model = NeuralNetwork(21).to(device)
    model.load_state_dict(torch.load(path+"EDFA"+EDFAID+".pth", weights_only=True))

    fibre_output_df = pd.DataFrame(fibre_output)
    combinedInputs = pd.concat([fibre_output_df, settings[["0", "1", "2", "3", "4"]]], axis=1)
    ...
    model.eval()
    edfa5_outputs = model(inputs_tensor)


    # FIBRE 5
    offChannelLoss, onChannelLoss, offChannelStd, onChannelStd = Fibre_ModelLoad.getLosses("fibre 5")
    fibre_output, _ = Fibre_ModelLoad.applyLosses(perms, offChannelLoss, onChannelLoss, offChannelStd, onChannelStd, edfa5_outputs.detach().cpu().numpy())
    ...
    model = NeuralNetwork(22).to(device)
    model.train()
    ...
    path = "models/ann/"
    torch.save(model.state_dict(), path+"EDFA6.pth")
