...
def trainModel(inputPowers, outputPowers, settings, perms, numTraining=300):
    ...
    model = NeuralNetwork(len(X_train[0])).to(device)
    model.train()

    optimizer = optim.Adam(model.parameters(), lr=0.00402)
    ...
    with torch.no_grad():
        for x,y in test_dataloader:
            x, y = x.to(device).to(torch.float32), y.to(device).to(torch.float32)
            target = model(x)
            preds = torch.cat((preds, target))
            actual = torch.cat((actual, y))

    preds = preds.cpu().numpy()
    actual = actual.cpu().numpy()
    error = np.nanmean(np.abs((actual - preds) * activeChannels[:len(actual)]))
