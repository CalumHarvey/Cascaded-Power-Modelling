from GP_Functions import EDFA1Model, EDFA2Model, EDFA3Model, EDFA4Model, EDFA5Model, EDFA6Model, EndToEnd
import pandas as pd
import numpy as np
import warnings
...
def trainAllModels(uncertain):
    """ Trains all models for cascaded learning in order from EDFA 1 to EDFA 6

    Args:
        uncertain (Boolean): True enables uncertainty propagation, False reverts to standard cascade
    """
    ...
    for x in range(6):
        ...
        path = "CascadedPowerModelling/data/EDFA "
        ...
        error = functionNames[x](inputPowers, outputPowers, settings, perms, uncertain)
        print(error)
...
def trainSingleEDFAModel(x, uncertain):
    """Trains only a single EDFA model

    Args:
        x (int): Index of EDFA to train from 1 to 6
        uncertain (Boolean): True enables uncertainty propagation
    """
...
if __name__ == "__main__":
    ...
    # Uncertainty propagating cascade 
    # errors = trainAllModels(True)
    # print(errors)

    # Standard cascade
    # errors = trainAllModels(False)
    # print(errors)
