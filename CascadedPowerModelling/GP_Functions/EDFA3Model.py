from GP_Functions import EDFA_ModelLoad, Fibre_ModelLoad
...
def trainModel(inputPowers, outputPowers, settings, perms, uncertain, numTraining=300):
    ...
    class ExactGPModel(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            if uncertain:
                ...
                self.covar_module = (gpytorch.kernels.ScaleKernel(LKUncertain.LaplacianKernel()) + gpytorch.kernels.ScaleKernel(UncertainKernel.UncertainKernel(ard_num_dims=19))).cuda()
            else:
                self.mean_module = gpytorch.means.ConstantMean()
                self.covar_module = (gpytorch.kernels.ScaleKernel(LK.LaplacianKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=19))).cuda()
    ...
    if uncertain:
        torch.save(model.state_dict(), path+"NIGP/EDFA3.pth")
        torch.save(train_data, path+"NIGP/EDFA3_trainingData.pt")
    else:
        torch.save(model.state_dict(), path+"GP/EDFA3.pth")
        torch.save(train_data, path+"GP/EDFA3_trainingData.pt")
