...
def trainModel(inputPowers, outputPowers, settings, perms, uncertain, numTraining=300):
    ...
    class ExactGPModel(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            if uncertain:
                ...
                self.covar_module = (gpytorch.kernels.ScaleKernel(LKUncertain.LaplacianKernel()) + gpytorch.kernels.ScaleKernel(UncertainKernel.UncertainKernel(ard_num_dims=21))).cuda()
            else:
                self.mean_module = gpytorch.means.ConstantMean()
                self.covar_module = (gpytorch.kernels.ScaleKernel(LK.LaplacianKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=21))).cuda()
    ...
    if uncertain:
        torch.save(model.state_dict(), path+"NIGP/EDFA5.pth")
        torch.save(train_data, path+"NIGP/EDFA5_trainingData.pt")
    else:
        torch.save(model.state_dict(), path+"GP/EDFA5.pth")
        torch.save(train_data, path+"GP/EDFA5_trainingData.pt")
